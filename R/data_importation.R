# Importation d'un fichier atp_matches_* du dépC4t tennis_atp ----
## Important : le dépC4t tennis_atp est supposé être au même niveau de l'aroboresence que le dépot dashboard_atp_tour

#### Configuration préliminaire ####
path <- "../tennis_atp/" # Chemin relatif vers le dépot tennis_atp depuis le dépot dashboard_atp_tour
...
# Choix arbitraire d'un fichier C  importer (exemple)
...
## Les champs minutes et stats de jeu des matches de 1968 C  1990 ne sont pas renseignés et importés en tant que logical
...
atp %>% 
  mutate(Année = stringr::str_sub(tourney_id, start=1L, end=4L)) %>%
  select(Année, tourney_id, tourney_name, tourney_level, tourney_date, tourney_type, surface, draw_size) %>%
...
#' @Description
#'
#'    Elle filtre le data frame en ne gardant que les lignes dont la  
#'    valeur de la colonne tourney_name n'est pas dans la liste names_to_exclude.
#' 
#' @Entrees
#' 
#'    group_tournoi : un data frame contenant au moins une colonne tourney_name.
#'    names_to_exclude : une liste de chaines de caracteres representant les noms de tournois a exclure.

#' @Sorties
#'    Un data frame filtre de la maniere decrite ci-dessus.

#' @Exemples
#'    Voici un exemple d'utilisation de la fonction filter_tournament_names :
#'    names_to_exclude <- c('Davis Cup', 'Masters Cup', 'Tour Finals', 'WCT Challenge Cup', 'Masters')
#'    group_tournoi <- filter_tournament_names(group_tournoi, names_to_exclude)
#' 
#'    Cet exemple cree une liste de noms de tournois a exclure, puis utilise cette liste pour filtrer le data frame group_tournoi 
#'    en ne gardant que les lignes dont la valeur de la colonne tourney_name n'est pas dans la liste.

filter_tournament_names <- function(group_tournoi, names_to_exclude) {
  for (name in names_to_exclude) {
    group_tournoi <- group_tournoi[!grepl(name, group_tournoi$tourney_name), ]
  }
  return(group_tournoi)
}
names_to_exclude <- c('Davis Cup', 'Masters Cup', 'Tour Finals', 'WCT Challenge Cup', 'Masters')
group_tournoi <- filter_tournament_names(group_tournoi, names_to_exclude)
...
fullJoin_tournoi_villes<-fullJoin_tournoi_villes %>% mutate("Grand_Chelem" = case_when(PAYS == "France" ~ "Roland Garros depuis 1891.",
                                                                                       PAYS == "UK" ~ "Wimbledon depuis 1877.",
                                                                                       PAYS=="USA" ~ "US Open depuis 1881.",
                                                                                       PAYS== "Australia" ~ "Australia Open depuis 1905.",
...
donnees_carte <- donnees_carte %>% 
  mutate(lat = ifelse(name == "Boo", 56.43, lat),
         long = ifelse(name == "Boo", 12.83, long),
         tourney_name = ifelse(name == "Boo", "Bastad", tourney_name),
         name = ifelse(name == "Boo","Bastad", name))

donnees_carte <- donnees_carte %>% 
  mutate(lat = ifelse(name == "Bahia Blanca", -34.6, lat),
         long = ifelse(name == "Bahia Blanca", -58.38, long),
         tourney_name = ifelse(name == "Bahia Blanca", "Buenos Aires", tourney_name),
         name = ifelse(name == "Bahia Blanca","Buenos Aires", name))

#Informations sur les infos bulles par la suite
library(htmltools)
mytext <- paste("Pays : ",  donnees_carte$PAYS,"<br/>",
                "Ville : ",  donnees_carte$name,"<br/>", 
                "Grand Chelem : ",  donnees_carte$Grand_Chelem, "<br/>", 
                "ATP 1000 : ",  donnees_carte$ATP_1000, "<br/>",
                "ATP 500 : ",  donnees_carte$ATP_500, "<br/>",
                "ATP 250 : ",  donnees_carte$ATP_250, "<br/>",
                "Nombre de rencontres ATP : ",  donnees_carte$RENCONTRES, 
                sep="") %>%
  lapply(htmltools::HTML)

## Tableau des règles

TabPoints <- data.frame(Année = c("1970", "1973", "1990", "2000","2009","2016","2022","2022"),
                        Règles = c(" Création de ATP Finals (dernier tournoi de l'année)","Commencement du classement ATP","Création des tournois Masters (actuellement Masters 1000)"," Apparition du Atp Race classement","Adoption du système de points actuel","Les Jeux olympiques et la Coupe Davis ne rapportent aucun point ATP","Les 4 tournois du Grand Chelem possèdent le jeu décisif en 10 points à 6 jeux partout lors du dernier set","Wimbledon joué sans points à cause de la décision du All England Club de bannir les Russes et les Biélorusses de  l'édition 2022."))

TabJeux <- data.frame(Année = c("1970", "1973", "2000", "2007","2019"),
                      Règles = c("Apparition du Tie-Break joué à 8 jeux","le Tie-break sera joué à 6 jeux","Le repos après le premier jeu de chaque set est supprimé"," Dernière finale en 5 sets pour les Masters 1000"," Le super tie-break fait son apparition à l'Open d'Australie"))

TabEquip <- data.frame(Année =c("1968","1972","1974","1977","1977","1978","1980","1980","1980","1984","1987","1994","1995","2006","2007","2012","2014"),
                       Règles = c("Apparition des raquettes en métal","L'US Open accepte des vêtements de couleur"," Changement de surface du US Open du gazon en terre battue","Création des courts Decoturf"," Us open en surface dur","La couleur des balles passe du blanc au jaune fluo","Apparition des raquettes en matériaux composites","Adoption des Machine Cyclops","Wimbledon reste le dernier tournoi du grand chlem à exiger les vêtements blancs","Disparition totale des raquettes en bois"," Changement de surface de AO du gazon en dur","Création des radars détectant la vitesse de service","Le blanc intégral est choisi pour Wimbledon","Adoption du Hawk-Eye","Création des courts Plexicushion","FFT impose la tenue blanche dans ses organisations sauf autorisation contraire","Apparition des raquettes intelligente/connectée"))
